...
the address. On Linux and macOS, a matching repeated start requests a fresh native
browser grant from the existing controller without restarting Gateway or Vite.
Busy or unready requests report a pending link; Windows repeats still report the
address.
